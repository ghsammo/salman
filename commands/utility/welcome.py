...
class Utility(commands.Cog):
    ...
    async def set_welcome_channel(self, ctx, channel: discord.TextChannel):
        try:
            async with self.bot.pool.acquire() as connection:
                # Use UPSERT to insert or update the welcome channel setting for this server
    ...
    async def on_member_join(self, member):
        if member.guild is None: # Ensure it's a guild member
            return

        welcome_channel_id = None
        try:
            async with self.bot.pool.acquire() as connection:
                # Retrieve the welcome channel ID for this server
            ...
            # Continue without sending a welcome message if there's a database error
            return
    ...
    @commands.command(name='servericon', help='Displays the server\'s icon.')
    async def servericon(self, ctx):
        if ctx.guild and ctx.guild.icon:
            # Adding a subtle emoji to the server icon embed title
    ...
    @commands.command(name='userinfo', help='<:user:1374846273706000434> Displays information about a user. Usage: .userinfo [user]')
